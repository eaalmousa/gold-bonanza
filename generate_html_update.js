...
// The new implementation of evaluateSniperSignal matching the bounce-then-fail TS rules
const newSniperSignal = `    function evaluateSniperSignal(tf1h, tf15m) {
  ...
  if (side === 'LONG') {
    ...
    return { kind: 'SNIPER', side: 'LONG', score, reasons, entryPrice: triggerPrice, stopLoss, takeProfit, qty, sizeUSDT: qty * triggerPrice, atr15: atr };
  } else {
    // ---- SHORT ANTI-EXHAUSTION LOGIC (BOUNCE-THEN-FAIL) ----
    const isBearCandle = close15 < open15;
    const candleAtrRatio = range / atr;
    
    // 1. Expansion Cap - prevent selling the immediate flush
    if (candleAtrRatio > 1.8) return null;

    // 2. Zone check - price must have bounced into / retested the EMAs
    ...
    if (extensionBelowZone > 1.0) return null; // Stale setup validation
    ...
    if (!(rsiNow < rsiPrev && (rsiPrev - rsiNow) >= 0.7)) return null; // momentum turning down
    score += 3; reasons.push('Bounce exhausted, RSI turning down');

    // 4. Structural Failure / Anatomy Rejection
    const bodyPct = (body / range) * 100;
    ...
    // Anti-bottom Proximity Check (Local Floor)
    ...
    const followThrough = close15 < prev.close;
    if (isBreakingFloor && (!cleanCloseBelow || !followThrough)) return null;

    // Lower high or rejection confirming weakness
    ...
    reasons.push('Lower high / Rejection structural failure');
    ...
    const balance = parseFloat(document.getElementById('balance')?.value || 300);
    const qty = (balance * ACTIVE_MODE.riskPct) / stopDistance;
    return { kind: 'SNIPER', side: 'SHORT', score, reasons, entryPrice: triggerPrice, stopLoss, takeProfit, qty, sizeUSDT: qty * triggerPrice, atr15: atr };
  }
}`;
...
if (startIdx === -1 || Math.abs(startIdx - html.lastIndexOf('    function evaluateSniperSignal(tf1h, tf15m) {')) > 10) {
  console.log('Error: evaluateSniperSignal found multiple times or not found');
} else if (endIdx !== -1) {
  // We need to replace up to the LAST brace before evaluateBreakoutSignal.
  // There is about 10 characters of whitespace before 'function evaluateBreakoutSignal'.
