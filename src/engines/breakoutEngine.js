...
exports.evaluateBreakoutSignal = evaluateBreakoutSignal;
const indicators_1 = require("./indicators");
const regimeFilter_1 = require("./regimeFilter");
function evaluateBreakoutSignal(tf1h, tf15m, activeMode, balance, regime, regimeScoreBonus, orderFlow, btc4hTrend, btcRegimeLabel, symbol) {
    if (!tf15m || tf15m.length < 90)
        return null;
    ...
    if (currentBreakout) {
        currentBreakout.entryType = 'PENDING_BREAKOUT';
    ...
    for (let lookback = 1; lookback <= MAX_RETEST_WAIT; lookback++) {
        ...
        if (pastBreakout) {
            const bLevel = pastBreakout.breakLevel;
            if (!bLevel) {
                pastBreakout.entryType = 'RETEST_CONFIRMED';
                return pastBreakout;
            }
            ...
            for (let j = pastSliceLength - 1; j < tf15m.length - 1; j++) {
                ...
                if (c.high > maxH)
                    maxH = c.high;
                if (side === 'LONG' && c.close < bLevel)
                    closedAgainst = true;
                if (side === 'SHORT' && c.close > bLevel)
                    closedAgainst = true;
            }
            ...
            if (closedAgainst) {
                pastBreakout.entryType = 'INVALIDATED';
            ...
            if (side === 'LONG') {
                if (minL <= bLevel * 1.002 || minL <= bLevel + (atr * 0.25))
                    hasRetested = true;
            }
            else {
                if (maxH >= bLevel * 0.998 || maxH >= bLevel - (atr * 0.25))
                    hasRetested = true;
            }
            if (hasRetested) {
                ...
                if (validLongRetest) {
                    ...
                    if (lookback === MAX_RETEST_WAIT) {
                        pastBreakout.entryType = 'EXPIRED_NO_RETEST';
                        return pastBreakout;
                    }
                    pastBreakout.entryType = 'PENDING_BREAKOUT';
                    return pastBreakout;
                }
            }
...
function evaluateCoreBreakout(tf1h, tf15m, activeMode, balance, regime, regimeScoreBonus, orderFlow, btc4hTrend, btcRegimeLabel, symbol) {
    const modeKey = activeMode.key;
    const debugLog = [`[BreakoutV3] ${symbol ?? ''}`];
    ...
    const ema20_1h = (0, indicators_1.calcEMA)(closes1h, 20);
    const ema50_1h = (0, indicators_1.calcEMA)(closes1h, 50);
    const ema200_1h = (0, indicators_1.calcEMA)(closes1h, 200);
    ...
    const ema20_15 = (0, indicators_1.calcEMA)(closes15, 20);
    const rsi14_15 = (0, indicators_1.calcRSI)(closes15, 14);
    const atr14_15 = (0, indicators_1.calcATR)(highs15, lows15, closes15, 14);
    const volSMA20_15 = (0, indicators_1.calcSMA)(vols15, 20);
    const volSMA50_15 = (0, indicators_1.calcSMA)(vols15, 50);
    const dollarVols15 = vols15.map((v, i) => v * closes15[i]);
    const dollarVolSMA20_15 = (0, indicators_1.calcSMA)(dollarVols15, 20);
    ...
    if ([rsiNow, atr, volAvg, e20].some(v => v == null))
        return null;
    ...
    if (side === 'LONG') {
        ...
        if (coilRange > cfg.coilRangePctMax)
            return null;
        score += 3;
        ...
        const entryTiming = lateExtension < 0.5 ? 'OPTIMAL' : lateExtension < 1.0 ? 'EARLY' : 'LATE';
        ...
        if (volRatio < cfg.volMult || volSpike < cfg.volSpikeMult)
            return null;
        let volScore = 2;
        if (volRatio > 2.0)
            volScore += 2;
        ...
        // ─── ORDER FLOW ─────────────────────────────────────
        const flowCheck = (0, regimeFilter_1.validateOrderFlow)(orderFlow, 'LONG');
        ...
        if (regimeScoreBonus && regimeScoreBonus > 0)
            reasons.push('Regime supportive');
        ...
        if (prev2) {
            ...
            if (accelPct > 0.002) {
                score += 1;
                reasons.push(`Downward accel`);
            }
        }
        const flowCheck = (0, regimeFilter_1.validateOrderFlow)(orderFlow, 'SHORT');
        const missingFlowPenalty = flowCheck.missingFlow ? 3 : 0;
        if (!flowCheck.ok && modeKey !== 'AGGRESSIVE')
            return null;
        score += flowCheck.score;
