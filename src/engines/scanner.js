...
exports.runBonanzaCore = runBonanzaCore;
const binanceApi_1 = require("../services/binanceApi");
const sniperEngine_1 = require("./sniperEngine");
const breakoutEngine_1 = require("./breakoutEngine");
const regimeFilter_1 = require("./regimeFilter");
const portfolioRisk_1 = require("./portfolioRisk");
const trading_1 = require("../types/trading");
const sniperEngine_2 = require("./sniperEngine");
const BATCH_SIZE = 8;
const BATCH_DELAY = 400;
const sleep = (ms) => new Promise(r => setTimeout(r, ms));
async function runBonanzaCore(symbols, activeMode, balance, onProgress, orderFlowSnapshots, onRegimeUpdate, currentOpenPositionCount, portfolio // For portfolio risk checks
) {
    ...
    try {
        const [btc1h, btc4h] = await Promise.all([
            (0, binanceApi_1.fetchKlines)('BTCUSDT', '1h', 220),
            (0, binanceApi_1.fetchKlines)('BTCUSDT', '4h', 100)
        ]);
        const detection = (0, regimeFilter_1.detectMarketRegime)(btc1h, btc4h);
    ...
    const corrLimit = (0, regimeFilter_1.getCorrelationPositionLimit)(regime, btc4hTrend, openCount);
    ...
    try {
        const res = await fetchWithTimeout(`${trading_1.SPOT_API}/api/v3/ticker/24hr`, 5000);
        if (res.ok) {
        ...
        try {
            const res = await fetchWithTimeout(`${trading_1.FUTURES_API}/fapi/v1/ticker/24hr`, 5000);
            if (res.ok) {
                const data = await res.json();
                for (const t of data)
                    tickers[t.symbol] = parseFloat(t.priceChangePercent) || 0;
    ...
    const signalsThisCycle = new Set();
    const defaultPortfolio = portfolio ?? {
        openPositions: [],
        currentScanCycleStart: Date.now()
    };
    function createTrace(symbol, engine, logs, sig, now) {
        const logsStr = logs.join(' | ');
        let status = 'REJECTED';
        let lastRejectReason = undefined;
        if (sig) {
            if (sig.entryType?.includes('PENDING') || sig.entryType?.includes('RETEST_FAILED'))
                status = 'PENDING';
            else
                status = 'ACCEPTED';
            if (sig.entryType === 'RETEST_FAILED' || sig.entryType === 'INVALIDATED')
                status = 'INVALIDATED';
            else if (sig.entryType === 'EXPIRED_NO_RETEST')
                status = 'EXPIRED';
        }
        else {
    ...
    for (let batch = 0; batch < symbols.length; batch += BATCH_SIZE) {
        ...
        const candidateResults = await Promise.allSettled(chunk.map(async (symbol) => {
            const [tf1h, tf15m] = await Promise.all([
                (0, binanceApi_1.fetchKlines)(symbol, '1h', 220),
                (0, binanceApi_1.fetchKlines)(symbol, '15m', 110)
            ]);
            ...
            const sniperLogStart = sniperEngine_2.globalDebugLogs.length;
            const sniper = (0, sniperEngine_1.evaluateSniperSignal)(tf1h, tf15m, activeMode, balance, regime, regimeScoreBonus, symbolFlow, btc4hTrend, regimeLabel, symbol);
            const currentSniperLogs = sniperEngine_2.globalDebugLogs.slice(sniperLogStart).find(l => l[0]?.includes(symbol)) || (sniper?.debugLog || []);
            const breakout = (0, breakoutEngine_1.evaluateBreakoutSignal)(tf1h, tf15m, activeMode, balance, regime, regimeScoreBonus, symbolFlow, btc4hTrend, regimeLabel, symbol);
        ...
        for (const result of candidateResults) {
            ...
            if (sniper?.signal && newSignalsThisScan < corrLimit.maxNewPositions) {
                const check = (0, portfolioRisk_1.checkPortfolioExposure)(symbol, sniper.signal.side, regime, btc4hTrend, defaultPortfolio, signalsThisCycle);
                if (check.allowed) {
            ...
            if (breakout?.signal && newSignalsThisScan < corrLimit.maxNewPositions) {
                ...
                if (breakout.signal.entryType === 'RETEST_CONFIRMED') {
                    const check = (0, portfolioRisk_1.checkPortfolioExposure)(symbol, breakout.signal.side, regime, btc4hTrend, defaultPortfolio, signalsThisCycle);
                    if (check.allowed) {
                        pipelineSignals.push({
                    ...
                    pipelineSignals.push({
                        symbol, signal: breakout.signal, price: breakout.price,
                        change24h: breakout.change24h, timestamp: breakoutTrace.timestamp,
                        id, status: breakoutTrace.status
                    });
                }
            }
        }
