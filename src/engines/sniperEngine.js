...
function evaluateSniperSignal(tf1h, tf15m, activeMode, balance, regime, regimeScoreBonus, orderFlow, btc4hTrend, btcRegimeLabel, symbol) {
    ...
    if (modeKey === 'AGGRESSIVE') {
        ...
        if (regime === 'TRENDING_DOWN') {
            debugLog.push('REJECT: Counter-trend regime (LONG in DOWN)');
            return null;
        }
    }
        ...
        if (regime === 'TRENDING_UP') {
            debugLog.push('REJECT: Counter-trend regime (SHORT in UP)');
            return null;
        }
    }
    ...
    const ema50_15 = (0, indicators_1.calcEMA)(closes15, 50);
    ...
    const macdResult = (0, indicators_1.calcMACD)(closes15);
    const bbResult = (0, indicators_1.calcBollingerBands)(closes15, 20, 2.0);
    const doublePattern = (0, indicators_1.detectDoublePattern)(highs15, lows15, closes15);
    ...
    if (side === 'LONG') {
        ...
        const entryTiming = extensionAboveZone < 0.20 ? 'OPTIMAL' :
        ...
        if (prev2) {
            ...
            if (accelPct > 0.0015) {
                score += 2;
                reasons.push(`Strong acceleration (+${(accelPct * 100).toFixed(3)}%)`);
            }
            else if (accelPct > 0)
                score += 1;
        }
        ...
        if (pctB != null) {
            if (pctB <= 0.15) {
                score += 2;
                reasons.push(`BB lower band (%B=${(pctB * 100).toFixed(0)}%) — oversold`);
            }
            else if (pctB <= 0.30) {
                score += 1;
                reasons.push(`Near BB lower (%B=${(pctB * 100).toFixed(0)}%)`);
            }
        }
        ...
        const finalTiming = extAbove < 0.20 ? 'OPTIMAL' : extAbove < 0.45 ? 'EARLY' : 'LATE';
        ...
        const finalTimingShort = extensionBelowZone < 0.25 ? 'OPTIMAL' : extensionBelowZone < 0.65 ? 'EARLY' : 'LATE';
        ...
        if (volRatio > 3.5)
            volScore += 2;
        score += volScore;
        reasons.push(`Bear volume (${volRatio.toFixed(2)}x)`);
